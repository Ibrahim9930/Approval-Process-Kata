...
namespace DoctorWho.Db.Utilities
{
    public static class FakerGenerator
    {
        private static Faker<T> CreateFaker<T>(int itemsCount) where T : class
        {
            ...
            int id = 1;
            faker
                .RuleFor("ModifiedOn", (f, en) => f.Date.Recent())
                .RuleFor("CreatedOn",
                    (f, en) => f.Date.Past(10, (DateTime?) en.GetType().GetProperty("ModifiedOn")?.GetValue(en)))
                .RuleFor("CreatedBy", (f, en) => f.Random.ArrayElement(new string[]
                {
                    "admin", "modify-user"
                }))
                .RuleFor("ModifiedBy", (f, en) => f.Random.ArrayElement(new string[]
                    {
                        "admin", "modify-user"
                    }
                ));
            if (typeof(T) == typeof(SeededAuthor))
            {
                faker
